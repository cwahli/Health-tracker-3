...
const newInstruction = `You are a fast visual food identification and localization agent. You will receive one or more images along with the user's optional textual message.
...
const newContent = content.substring(0, startIndex + startMarker.length) + newInstruction + content.substring(endIndex);
fs.writeFileSync(filepath, newContent, 'utf8');
console.log('Fixed scoutSystemInstruction in server.ts for real');
