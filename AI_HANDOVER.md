...
- Injected strict prompt guardrails into the Vision Scout prompt (`SEMANTIC ALIGNMENT RULE`, `Visual Cross-Referencing Rule`, and `Confidence Scoring Calibration`) and the Nutrition Agent prompt (`Local Language Priority`, `Protein Verification`, `Strict Anti-Merging`, `Zero Hallucination`, and `Trace Nutrients Taxonomy`) in `server.ts` to enforce originalName preservation as absolute ground truth, handle protein names, restrict DB ID hallucination, support fungi categorization, and prevent upstream vision errors from corrupting calculations.
